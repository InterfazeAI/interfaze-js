...
save("basic.json", await client.chat.completions.create({
  model: "interfaze-beta",
  messages: [{ role: "user", content: "Say hi in one short sentence." }],
  max_tokens: 60,
}));

save("json_object.json", await client.chat.completions.create({
  model: "interfaze-beta",
  messages: [{ role: "user", content: "Return a JSON object with keys city and temp_c for Tokyo." }],
  response_format: { type: "json_object" },
}));

save("task_ocr.json", await client.chat.completions.create({
  model: "interfaze-beta",
  messages: [
...
save("precontext.json", await client.chat.completions.create({
  model: "interfaze-beta",
...

{
  ...
  const s = await client.chat.completions.create({
    model: "interfaze-beta", stream: true, messages: [{ role: "user", content: "Count 1 to 5." }],
  });
...

{
  const chunks: unknown[] = [];
  const s = await client.chat.completions.create({
    model: "interfaze-beta", stream: true,
